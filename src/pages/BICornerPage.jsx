...
import { useBICorner } from '../hooks';
import { getBICornerStats } from '../services/bicornerService';
import BICornerStats from '../components/BICorner/BICornerStats';
import BICornerChart from '../components/BICorner/BICornerChart';
import BICornerTable from '../components/BICorner/BICornerTable';
...
function BICornerPage() {
    ...
    const { data: visits, loading, error, refetch } = useBICorner(dateRange);

    React.useEffect(() => {
        if (visits) {
            getBICornerStats(dateRange).then(result => { if (result.data) setStats(result.data); });
        }
    }, [visits, dateRange]);
    ...
    if (loading && !visits) return <LoadingPage message="Memuat data kunjungan BI Corner..." />;
        ...
        <div className="space-y-6">
            ...
            <AnimatePresence mode="wait">
                <motion.div key={activeTab} variants={tabContentVariants} initial="enter" animate="center" exit="exit" transition={{ type: "spring", stiffness: 200, damping: 25 }}>
                    {activeTab === 'overview' && (
                        <div className="space-y-6">
                            <BICornerStats stats={stats} todayVisits={todayVisits} />
                            <VisitTrendChart visits={visits} title="Trend Kunjungan 7 Hari Terakhir" color="#14b8a6" />
                            {stats?.facultyDistribution && <BICornerChart data={stats.facultyDistribution} title="Distribusi Kunjungan per Fakultas" />}
                        </div>
                    )}
                    {activeTab === 'logs' && visits && <BICornerTable visits={visits} title="Log Kunjungan BI Corner" />}
                </motion.div>
            </AnimatePresence>
        </div>
    );
