...
import { SmartLabStats, SmartLabChart, SmartLabTable } from '../components/SmartLab';
import { LoadingPage, ErrorMessage, RefreshButton, DateRangePicker, LastUpdated, Tabs, VisitTrendChart } from '../components/Common';
import { useSmartLab } from '../hooks';
import { getSmartLabStats } from '../services/smartlabService';
...
function SmartLabPage() {
    ...
    const { data: visits, loading, error, refetch } = useSmartLab(dateRange);

    React.useEffect(() => {
        if (visits) {
            getSmartLabStats(dateRange).then(result => { if (result.data) setStats(result.data); });
        }
    }, [visits, dateRange]);
    ...
    if (loading && !visits) return <LoadingPage message="Memuat data kunjungan SmartLab..." />;
        ...
        <div className="space-y-6">
            ...
            <AnimatePresence mode="wait">
                <motion.div key={activeTab} variants={tabContentVariants} initial="enter" animate="center" exit="exit" transition={{ type: "spring", stiffness: 200, damping: 25 }}>
                    {activeTab === 'overview' && (
                        <div className="space-y-6">
                            <SmartLabStats stats={stats} todayVisits={todayVisits} />
                            <VisitTrendChart visits={visits} title="Trend Kunjungan 7 Hari Terakhir" color="#14b8a6" />
                            {stats?.facultyDistribution && <SmartLabChart data={stats.facultyDistribution} title="Distribusi Kunjungan per Fakultas" />}
                        </div>
                    )}
                    {activeTab === 'logs' && visits && <SmartLabTable visits={visits} title="Log Kunjungan SmartLab" />}
                </motion.div>
            </AnimatePresence>
        </div>
    );
