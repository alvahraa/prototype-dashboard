 ...
 * Audiovisual Service
 * 
 * Service untuk data kunjungan ruang audiovisual dari database.
 */

import { fetchBackendApi } from './api';

/**
 * Get all audiovisual visits from database
 * @param {Object} options - { startDate, endDate }
 * @returns {Promise<{data: Array, error: string|null}>}
 */
export async function getAudiovisualVisits(options = {}) {
    const params = new URLSearchParams();
    params.append('ruangan', 'audiovisual');
...
/**
 * Get today's audiovisual visits
 * @returns {Promise<{data: Array, error: string|null}>}
 */
export async function getTodayAVVisits() {
    const today = new Date();
    today.setHours(0, 0, 0, 0);

    return getAudiovisualVisits({
...
/**
 * Get audiovisual visit statistics
 * @param {Object} options - { startDate, endDate }
 * @returns {Promise<{data: Object, error: string|null}>}
 */
export async function getAVStats(options = {}) {
    const { data: visits, error } = await getAudiovisualVisits(options);
