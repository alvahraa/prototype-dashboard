 ...
 * Referensi Service
 * 
 * Service untuk data kunjungan ruang referensi dari database.
 */

import { fetchBackendApi } from './api';

/**
 * Get all referensi visits from database
 * @param {Object} options - { startDate, endDate }
 * @returns {Promise<{data: Array, error: string|null}>}
 */
export async function getReferensiVisits(options = {}) {
    const params = new URLSearchParams();
    params.append('ruangan', 'referensi');
...
/**
 * Get today's referensi visits
 * @returns {Promise<{data: Array, error: string|null}>}
 */
export async function getTodayRefVisits() {
    const today = new Date();
    today.setHours(0, 0, 0, 0);

    return getReferensiVisits({
...
/**
 * Get referensi visit statistics
 * @param {Object} options - { startDate, endDate }
 * @returns {Promise<{data: Object, error: string|null}>}
 */
export async function getRefStats(options = {}) {
    const { data: visits, error } = await getReferensiVisits(options);
