 ...
 * SmartLab Service
 * 
 * Service untuk data kunjungan SmartLab dari database.
 */

import { fetchBackendApi } from './api';

/**
 * Get all SmartLab visits from database
 * @param {Object} options - { startDate, endDate }
 * @returns {Promise<{data: Array, error: string|null}>}
 */
export async function getSmartLabVisits(options = {}) {
    const params = new URLSearchParams();
    params.append('ruangan', 'smartlab');
...
/**
 * Get today's SmartLab visits
 * @returns {Promise<{data: Array, error: string|null}>}
 */
export async function getTodaySmartLabVisits() {
    const today = new Date();
    today.setHours(0, 0, 0, 0);

    return getSmartLabVisits({
...
/**
 * Get SmartLab visit statistics
 * @param {Object} options - { startDate, endDate }
 * @returns {Promise<{data: Object, error: string|null}>}
 */
export async function getSmartLabStats(options = {}) {
    const { data: visits, error } = await getSmartLabVisits(options);
